...
- **Hyperparameters Tuning:** 
  - **Daily:** 1, 
  - **Weekly:** average of 5 and 7, 
  - **Monthly:** the average of 22 and 30
- **Result ( Most Common):** **Daily**= 1, **Weekly**= 5, **Monthly**= 30
...
- **Result (Most Common):** lag = 1 and 2
...
- **Result (Most Common):** 0.97 threshold
...
- **Result (Most Common):** 10-day window 
...
def calculate_performance_metrics():
    ...
    for model in model_names:
        if model == 'GAT':
            y_pred = pd.read_csv('GAT_prediction_panel.csv', index_col=0)
        else:
            y_pred = pd.read_csv(f'{model}_predictions.csv', index_col=0)
            
        actual = pd.read_csv('real_volatility.csv', index_col=0)
        y_true = actual.reindex(index=y_pred.index) 

        Y, Ŷ = y_true.values, y_pred.values
        eps = 1e-12
...
The cross-sectional evaluation on the last 10% of data shows clear performance differences. GAT performs best across all metrics: RMSE of 0.001236, RMSPE of 0.57, QLIKE of 0.092128, and MAPE of 0.46, as shown in @tbl-performance

Traditional models perform well: HAR_RV and PCA_Linear show similar results (RMSE ~0.0017, RMSPE ~0.60), while Linear model performs comparably. LAG model shows moderate performance with higher relative errors.

Machine learning models show mixed results: Random_Forest has reasonable accuracy but higher relative errors (RMSPE: 1.50, MAPE: 1.21), while Gradient_Boosting underperforms significantly (RMSPE: 2.48, MAPE: 2.00).

The results show GAT's effectiveness in capturing complex volatility patterns, while traditional models remain competitive despite simpler structures.
...
def load_and_align_data():
    ...
    return model_preds, aligned_actual, shared_stocks, shared_dates
...
def create_comparison_plots():
    ...
    for idx, model in enumerate(model_names):
        ...
        # Calculate RMSE
        rmse = np.sqrt(np.mean((avg_pred - avg_actual)**2))
        ...
        axes[idx].set_title(f'{model} (RMSE: {rmse:.6f})', fontsize=10)
...
In less volatile conditions, the model maintains a tight fit, with fluctuations exhibiting minimal residuals. This consistency is quantified by an overall test Root Mean Square Error (RMSE) of 0.001252, as indicated in the figure title. It is evident that the majority of prediction error is concentrated within a limited number of spike windows. 
